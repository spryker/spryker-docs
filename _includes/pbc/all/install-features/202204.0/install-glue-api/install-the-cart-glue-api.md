...
This document describes how to integrate the [Cart](/docs/pbc/all/cart-and-checkout/cart-feature-overview/cart-feature-overview.html) feature API into a Spryker project.
...
1. [Create a guest cart](/docs/pbc/all/cart-and-checkout/manage-using-glue-api/manage-carts-of-registered-users/manage-carts-of-registered-users.html#create-a-guest-cart).
2 Send the request: `GET https://glue.mysprykershop.com/carts/`.
1. Check that the response contains the cart you have created.

Ensure that the `items` resource relationships are registered as a relationship of the `carts` resource:

1. [Add one or more items to the cart](/docs/pbc/all/cart-and-checkout/manage-using-glue-api/manage-carts-of-registered-users/manage-items-in-carts-of-registered-users.html#add-an-item-to-a-registered-users-cart).
...
1. [Add one or more items to the cart](/docs/pbc/all/cart-and-checkout/manage-using-glue-api/manage-guest-carts/manage-guest-cart-items.html#add-items-to-a-guest-cart).
