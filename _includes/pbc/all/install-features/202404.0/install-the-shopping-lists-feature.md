...
| Cart                        | {{page.version}} | [Cart feature integration](/docs/scos/dev/feature-integration-guides/{{site.version}}/cart-feature-integration.html)                                                    |
...
Make sure that the changes were implemented successfully. For this purpose, trigger the following methods and make sure
that the above events have been triggered:
...
This step will publish tables on change (create, edit, delete) to the `spy_shopping_list_storage` and synchronize the
data to Storage.
...
| PLUGIN                                    | SPECIFICATION                                    | PREREQUISITES | NAMESPACE                                                            |
|-------------------------------------------|--------------------------------------------------|---------------|----------------------------------------------------------------------|
| ShoppingListSynchronizationDataBulkPlugin | Allows populating empty storage table with data. | None          | Spryker\Zed\ShoppingListStorage\Communication\Plugin\Synchronization |
...
Verify if `console sync:data --help` has `shopping_list_customer` as an available resource in the list. Make sure
that `sync.storage.shopping_list` and `sync.storage.shopping_list.error` queues get created after at least one message
is pushed through Spryker into any RabbitMQ queue.
...
or removed) to Redis accordingly.
...
Make sure that in the database the permission groups are added to the `spy_shoppping_list_permission_group` table and
the permissions are added to the `spy_permission table`; the relations between them are added to
the `spy_shopping_list_permission_group_to_permission` table.
...
The following imported entities will be used as shopping lists being directly shared with specific company users in
Spryker OS.
...
The following imported entities will be used as shopping lists being directly shared with specific company business
units in Spryker OS.
...
| ShoppingListCompanyUserPreDeletePlugin                            | Removes company user relations from shared shopping lists.                                                                                                                                                                                                                                                                                                       | None                                                                                                                                                 | Spryker\Zed\ShoppingList\Communication\Plugin\CompanyUser         |
| CompanyBusinessUnitPreDeletePlugin                                | Remove company business unit from shared shopping list.                                                                                                                                                                                                                                                                                                          | None                                                                                                                                                 | Spryker\Zed\ShoppingList\Communication\Plugin\CompanyBusinessUnit |
| ShoppingListItemProductConcreteHasValidStoreAddItemPreCheckPlugin | Go to the product page and select a concrete product. Then open another tab and log into the Back Office. Find a concrete product selected on PDP by a concrete SKU. Uncheck the current store option on the product edit page. Press "Add to shopping list" on the PDP tab to check that the product store validation is enabled. An error message will appear. | None                                                                                                                                                 | Spryker\Zed\ShoppingList\Communication\Plugin\ShoppingList        |
| ShoppingListItemCollectionNoteExpanderPlugin                      | Populates shopping list item notes.                                                                                                                                                                                                                                                                                                                              | None                                                                                                                                                 | Spryker\Zed\ShoppingListNote\Communication\Plugin                 |
| ShoppingListItemNoteBulkPostSavePlugin                            | Creates, updates or deletes note for shopping list item.                                                                                                                                                                                                                                                                                                         | None                                                                                                                                                 | Spryker\Zed\ShoppingListNote\Communication\Plugin                 |
| SanitizeCustomerShoppingListsImpersonationSessionFinisherPlugin   | Removes customer shopping list collection from session.                                                                                                                                                                                                                                                                                                          | None                                                                                                                                                 | Spryker\Client\ShoppingListSession\Plugin\Agent                   |
...
Share User A's shopping list with User B, then change the shopping list from User A's profile; User B should get the
updated shopping list. Also, make sure that sharing shopping lists have the correct permission group (the ones you had
...
| PLUGIN                                | SPECIFICATION                                                | PREREQUISITES | NAMESPACE                                                                                |
|---------------------------------------|--------------------------------------------------------------|---------------|------------------------------------------------------------------------------------------|
| ShoppingListPageRouteProviderPlugin   | Adds the shopping list routes to the Yves application        | None          | \SprykerShop\Yves\ShoppingListPage\Plugin\Router                                         |
| ShoppingListWidgetRouteProviderPlugin | Adds the shopping list widget routes to the Yves application | None          | \SprykerShop\Yves\ShoppingListWidget\Plugin\Router\ShoppingListWidgetRouteProviderPlugin |
...
| PLUGIN                                          | SPECIFICATION                                            | PREREQUISITES | NAMESPACE                                                  |
|-------------------------------------------------|----------------------------------------------------------|---------------|------------------------------------------------------------|
| ShoppingListQuickOrderFormHandlerStrategyPlugin | Adds products to shopping list with quick order feature. | None          | \SprykerShop\Yves\ShoppingListWidget\Plugin\QuickOrderPage |
...
| PLUGIN                                 | SPECIFICATION                                                                   | PREREQUISITES | NAMESPACE                                                       |
|----------------------------------------|---------------------------------------------------------------------------------|---------------|-----------------------------------------------------------------|
| ShoppingListItemNoteFormExpanderPlugin | Adds ShoppingListItemNote form fields to builder using ShoppingListItemNoteForm | None          | SprykerShop\Yves\ShoppingListNoteWidget\Plugin\ShoppingListPage |
