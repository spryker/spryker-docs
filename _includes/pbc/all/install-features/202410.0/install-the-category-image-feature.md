...
To configure export to key-value storage (Redis or Valkey), follow steps in the subsections.
...
| PLUGIN | SPECIFICATION | PREREQUISITES | NAMESPACE |
| --- | --- | --- | --- |
| CategoryImageSynchronizationDataPlugin | Synchronizes all category image entries from the database to key-value storage (Redis or Valkey). | None | Spryker\Zed\CategoryImageStorage\Communication\Plugin\Synchronization |
...
Make sure that when a category image is created, updated, or deleted, it's exported or removed from key-value storage (Redis or Valkey) accordingly.

{% endinfo_block %}

| STORAGE TYPE | TARGET ENTITY | EXAMPLE EXPECTED DATA IDENTIFIER |
| --- | --- | --- |
| Redis or Valkey | Category Image | `kv:category_image:en_us:1` |
