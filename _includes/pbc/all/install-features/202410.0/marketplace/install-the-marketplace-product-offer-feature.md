...
To configure export to key-value storage (Redis or Valkey) and Elasticsearch, take the following steps:
...
- `console sync:data product_concrete_product_offers` exports data from the `spy_product_concrete_product_offers_storage` table to key-value storage (Redis or Valkey).
- `console sync:data product_offer` exports data from the `spy_product_offer_storage` table to key-value storage (Redis or Valkey).
