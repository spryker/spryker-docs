...
Table `spy_sales_order_amendment_quote` is not used yet. This table could be used as a temporary storage for the quote
...
|-------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------|
| OrderAmendmentCartReorderValidatorPlugin                    | Validates if quote amendment order reference matches `CartReorderTransfer.order.orderReference`.                                                                                                        | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\SalesOrderAmendment\Communication\Plugin\CartReorder                     |
...
| IsAmendableOrderCartReorderValidatorRulePlugin              | Validates if all order items are in order item state that has `amendable` flag.                                                                                                                         | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\SalesOrderAmendmentOms\Communication\Plugin\CartReorder                  |
...
| OrderAmendmentCheckoutPreCheckPlugin                        | Validates if order is in a state that allows amendment.                                                                                                                                                 | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\SalesOrderAmendmentOms\Communication\Plugin\Checkout                     |
| CustomerOrderSavePlugin                                     | Saves customer info to sales related table.                                                                                                                                                             | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\Customer\Communication\Plugin\Checkout                                   |
| UpdateOrderByQuoteCheckoutDoSaveOrderPlugin                 | Updates order billing address with billing address data from quote.                                                                                                                                     | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\Sales\Communication\Plugin\Checkout                                      |
| OrderTotalsSaverPlugin                                      | Saves order totals.                                                                                                                                                                                     | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\Sales\Communication\Plugin\Checkout                                      |
| ReleaseUsedCodesCheckoutDoSaveOrderPlugin                   | Otherwise decreases the number of uses of each of found discount codes by 1.                                                                                                                            | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\Discount\Communication\Plugin\Checkout                                   |
| ShipmentTypeCheckoutDoSaveOrderPlugin                       | Creates/updates sales shipment type entity.                                                                                                                                                             | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\SalesShipmentType\Communication\Plugin\Checkout                          |
| UpdateCartNoteCheckoutDoSaveOrderPlugin                     | Updates order's cart note with the cart note provided in `QuoteTransfer.cartNote`.                                                                                                                      | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\CartNote\Communication\Plugin\Checkout                                   |
| ReplaceSalesOrderDiscountsCheckoutDoSaveOrderPlugin         | Deletes sales discount and sales discount code entities related to provided sales order ID. Iterates over `orderItems`, `orderExpenses` and creates sales discount entities for each item.              | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\Discount\Communication\Plugin\Checkout                                   |
| ReplaceSalesOrderShipmentCheckoutDoSaveOrderPlugin          | Recreates new sales shipment expenses for each item level shipment.                                                                                                                                     | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\Shipment\Communication\Plugin\Checkout                                   |
| SalesOrderAmendmentItemsCheckoutDoSaveOrderPlugin           | Replaces order item during amendment process.                                                                                                                                                           | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\SalesOrderAmendment\Communication\Plugin\Checkout                        |
| ProductBundleOrderSaverPlugin                               | Saves order bundle items.                                                                                                                                                                               | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\ProductBundle\Communication\Plugin\Checkout                              |
| ReplaceSalesOrderPaymentCheckoutDoSaveOrderPlugin           | Saves order payments from `QuoteTransfer`.                                                                                                                                                              | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\SalesPayment\Communication\Plugin\Checkout                               |
| GiftCardPaymentCheckoutDoSaveOrderPlugin                    | Iterates over `QuoteTransfer.payments` and saves gift card related payments into the `spy_payment_gift_card` DB table.                                                                                  | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\GiftCard\Communication\Plugin\Checkout                                   |
| ReplaceSalesOrderThresholdExpensesCheckoutDoSaveOrderPlugin | Iterates over `QuoteTransfer.expenses` and stores expenses of the type defined by {@link \Spryker\Shared\SalesOrderThreshold\SalesOrderThresholdConfig::THRESHOLD_EXPENSE_TYPE} in the database.        | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\SalesOrderThreshold\Communication\Plugin\Checkout                        |
...
| DummyPaymentCheckoutPostSavePlugin                          | If QuoteTransfer.billingAddress.lastName is 'Invalid' the plugin adds the error into CheckoutResponseTransfer.                                                                                          | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\DummyPayment\Communication\Plugin\Checkout                               |
| CloseQuoteRequestCheckoutPostSaveHookPlugin                 | If quote contains quote request version reference - marks quote request as closed.                                                                                                                      | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\QuoteRequest\Communication\Plugin\Checkout                               |
| SendEmailToGiftCardUser                                     | Sends an email to a Gift Card user.                                                                                                                                                                     | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\GiftCardMailConnector\Communication\Plugin\Checkout                      |
| PaymentAuthorizationCheckoutPostSavePlugin                  | Checks whether a payment method that requires authorization is selected for the given order.                                                                                                            | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\Payment\Communication\Plugin\Checkout                                    |
| PaymentConfirmPreOrderPaymentCheckoutPostSavePlugin         | Send a request to the used PSP App to confirm the pre-order payment.                                                                                                                                    | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\Payment\Communication\Plugin\Checkout                                    |
| DisallowQuoteCheckoutPreSavePlugin                          | Disallows quote checkout for the configured amount of seconds.                                                                                                                                          | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\QuoteCheckoutConnector\Communication\Plugin\Checkout                     |
| SalesOrderExpanderPlugin                                    | Transforms provided cart items according configured cart item transformer strategies.                                                                                                                   | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\Sales\Communication\Plugin\Checkout                                      |
| OriginalOrderQuoteExpanderCheckoutPreSavePlugin             | Sets `QuoteTransfer.originalOrder` with found order entity.                                                                                                                                             | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\SalesOrderAmendment\Communication\Plugin\Checkout                        |
...
| CountriesCheckoutDataValidatorPlugin                        | Verifies if countries can be found by countryIso2Codes given in `CheckoutDataTransfer.shipments.shippingAddress`.                                                                                       | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\Country\Communication\Plugin\CheckoutRestApi                             |
| ShipmentMethodCheckoutDataValidatorPlugin                   | Verifies if shipment method is valid.                                                                                                                                                                   | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\ShipmentsRestApi\Communication\Plugin\CheckoutRestApi                    |
| ItemsCheckoutDataValidatorPlugin                            | Validates if `CheckoutDataTransfer` provides shipment data per item level.                                                                                                                              | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\ShipmentsRestApi\Communication\Plugin\CheckoutRestApi                    |
| CustomerAddressCheckoutDataValidatorPlugin                  | Checks if customer addresses exists.                                                                                                                                                                    | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\CustomersRestApi\Communication\Plugin\CheckoutRestApi                    |
| CompanyBusinessUnitAddressCheckoutDataValidatorPlugin       | Checks if company addresses exists.                                                                                                                                                                     | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\CompanyBusinessUnitAddressesRestApi\Communication\Plugin\CheckoutRestApi |
| ShipmentTypeCheckoutDataValidatorPlugin                     | Validates whether shipment type related to the shipment method is active and belongs to the quote store.                                                                                                | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\ShipmentTypesRestApi\Communication\Plugin\CheckoutRestApi                |
| ClickAndCollectExampleReplaceCheckoutDataValidatorPlugin    | Replaces filtered product offers with suitable product offers from Persistence.                                                                                                                         | `Quote.quoteProcessFlow.name` === 'order-amendment'                                                                                              | Spryker\Zed\ClickAndCollectExample\Communication\Plugin\CheckoutRestApi              |
...
Make sure that that order editing with different types of products is available for the customer.
