...
Make sure that when you edit an order, `originalSalesOrderItems` are added to the JSON data in the `spy_quote.quote_data` database column of the corresponding quote.
...
Define that batch availability check facade method will be used.
...
| ProductOfferOriginalSalesOrderItemGroupKeyExpanderPlugin               | Expands provided group key with product offer reference if `ItemTransfer.productOfferReference` is set.                                                                                                    |                                                                                                                                             | Spryker\Service\ProductOffer\Plugin\SalesOrderAmendment                              |
...
| OrderAmendmentProductApprovalCartPreCheckPlugin                        | Checks the approval status for products. Skips check for items that are part of the original order.                                                                                                        |                                                                                                                                             | Spryker\Zed\ProductApproval\Communication\Plugin\Cart                                |
| OrderAmendmentProductApprovalPreReloadItemsPlugin                      | Checks and removes not approved product items from Quote transfer. Skips items that are part of the original order.                                                                                        |                                                                                                                                             | Spryker\Zed\ProductApproval\Communication\Plugin\Cart                                |
| OrderAmendmentProductBundleAvailabilityCartPreCheckPlugin              | Checks if the product bundle items in the `CartChangeTransfer` are available and active. Skips `isActive` validation for items with SKUs from `CartChangeTransfer.quote.originalSalesOrderItems`.          |                                                                                                                                             | Spryker\Zed\ProductBundle\Communication\Plugin\Cart                                  |
| OrderAmendmentProductBundleStatusCartPreCheckPlugin                    | Checks if the product bundle items in the `CartChangeTransfer` are active. Skips validation for items with SKUs from `CartChangeTransfer.quote.originalSalesOrderItems`.                                   |                                                                                                                                             | Spryker\Zed\ProductBundle\Communication\Plugin\Cart                                  |
| OrderAmendmentProductExistsCartPreCheckPlugin                          | Checks added to cart products on existing. Skips items that are part of the original order.                                                                                                                |                                                                                                                                             | Spryker\Zed\ProductCartConnector\Communication\Plugin\Cart                           |
...
| OrderAmendmentProductDiscontinuedCartPreCheckPlugin                    | Checks all items related products from cart change request if they are not discontinued. Skips items that are part of the original order.                                                                  |                                                                                                                                             | Spryker\Zed\ProductDiscontinued\Communication\Plugin\Cart                            |
| OrderAmendmentFilterInactiveProductOfferPreReloadItemsPlugin           | Checks and removes inactive product offers from the cart. Skips inactive product offers that are part of the original order.                                                                               |                                                                                                                                             | Spryker\Zed\ProductOffer\Communication\Plugin\Cart                                   |
| OrderAmendmentProductOfferCartPreCheckPlugin                           | Check if cart items product offer belongs to product. Skips product offers that are part of the original order.                                                                                            |                                                                                                                                             | Spryker\Zed\ProductOffer\Communication\Plugin\Cart                                   |
| OriginalOrderBundleItemCartPreReorderPlugin                            | Expands `CartReorderTransfer.quote` with original sales order items from the provided `CartReorderTransfer.order.bundleItems`.                                                                             |                                                                                                                                             | Spryker\Zed\ProductBundle\Communication\Plugin\CartReorder                           |
| OriginalSalesOrderItemCartPreReorderPlugin                             | Expands `CartReorderTransfer.quote` with original sales order items from the provided `CartReorderTransfer.order.items`.                                                                                   |                                                                                                                                             | Spryker\Zed\SalesOrderAmendment\Communication\Plugin\CartReorder                     |
...
| OrderAmendmentProductApprovalCheckoutPreConditionPlugin                | Returns `false` response if at least one quote item transfer has items with not approved product. Skips validation for items that are part of the original order.                                          |                                                                                                                                             | Spryker\Zed\ProductApproval\Communication\Plugin\Checkout                            |
...
| OrderAmendmentProductExistsCheckoutPreConditionPlugin                  | Validates if concrete products with sku `QuoteTransfer.item.sku` exist and active. Skips items that are part of the original order.                                                                        |                                                                                                                                             | Spryker\Zed\ProductCartConnector\Communication\Plugin\Checkout                       |
...
| OrderAmendmentProductOfferCheckoutPreConditionPlugin                   | Returns `false` response if at least one quote item transfer has items with inactive or not approved ProductOffer. Skips product offers that are part of the original order.                               |                                                                                                                                             | Spryker\Zed\ProductOffer\Communication\Plugin\Checkout                               |
| SalesOrderAmendmentQuoteCheckoutDoSaveOrderPlugin                      | Creates a new sales order amendment quote entity based on the provided `QuoteTransfer`.                                                                                                                    |                                                                                                                                             | Spryker\Zed\SalesOrderAmendment\Communication\Plugin\Checkout                        |
| UpdateDeletedItemReservationCommandByOrderPlugin                       | Retrieves the sales order amendment quote collection by order reference. If the collection is not empty, it updates the reservations of deleted items in the order.                                        |                                                                                                                                             | Spryker\Zed\SalesOrderAmendmentOms\Communication\Plugin\Oms                          |
...
- Deactivated/discontinued products or products without active offer should be available for order amendment.
...
| OrderAmendmentItemLinkWidget  | Handles url generation for deactivated products from order amendment. |               | SprykerShop\Yves\SalesOrderAmendmentWidget\Widget |
...
| OrderAmendmentItemLinkWidget   | Make sure the product url does not exists for deactivated products on the cart page. |
