...
This document describes how. to integrate the [Content Items](/docs/pbc/all/content-management-system/latest/base-shop/navigation-feature-overview.html) feature.
...
```bash
composer require spryker-feature/content-item:"{{page.release_tag}}" --update-with-dependencies
```
...
| PLUGIN  | SPECIFICATION  | PREREQUISITES | NAMESPACE   |
| ---------------- | ------------------- | ---------- | -------------------- |
| ContentStorageSynchronizationDataPlugin | Syncs all the content entries from the database to the key-value store (Redis or Valkey). |               | Spryker\Zed\ContentStorage\Communication\Plugin\Synchronization |
...
Make sure that, when content banners are exported or created, updated, deleted in the Back Office, they are exported or removed from the key-value store (Redis or Valkey) accordingly.

| STORAG TYPE                       | TARGET ENTITY    | EXAMPLE EXPECTED DATA IDENTIFIER        |
|-----------------------------------| ---------------------------- | ---------------------------------------------- |
| Key-value store (Redis or Valkey) | Content Banner                | content:en_us:br1                                 |
| Key-value store (Redis or Valkey) | Content Abstract Product List | content:en_us:apl2                                |
| Key-value store (Redis or Valkey) | Content Product Set           | content:en_us:ps-1                                |
| Key-value store (Redis or Valkey) | Content File List             | content:en_us:0d9f4722-d076-5acc-9d8e-e9daff7cd61 |
