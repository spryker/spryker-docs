...
```bash
composer require spryker-feature/file-manager:"202507.0" --update-with-dependencies
```
...
When a file data is created, updated, or deleted, make sure it's exported or removed from the key-value store (Redis or Valkey) accordingly.
