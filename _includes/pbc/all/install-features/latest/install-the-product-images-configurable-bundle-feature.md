...
| NAME | VERSION |
| --- | --- |
| Configurable Bundle  | {{site.versionid}} |
| Product Images | {{site.versionid}} |
...
2. Make sure that after step #4  or after `console sync:data configurable_bundle_template_image` command execution, the data s added to the `spy_configurable_bundle_template_image_storage` table and the key-value store (Redis or Valkey).
3. Make sure that when a product image set with the `fk_resource_configurable_bundle_template` is created or edited through ORM, it's exported to the key-value store (Redis or Valkey) accordingly.
...
| Key-value store (Redis or Valkey) | `ConfigurableBundleTemplateImage` | `kv:configurable_bundle_template_image:1` |
...
**Example expected data fragment for the key-value store (Redis or Valkey)**
...
Make sure that the latest data is present at Elasticsearch and the key-value store (Redis or Valkey) documents for configurable bundle templates.
