...
```bash
composer require spryker-feature/marketplace-product:"202507.0" --update-with-dependencies
```
...
| MerchantUpdatePublisherPlugin                                | Publishes the product by merchant IDs to the key-value store (Redis or Valkey). |           | Spryker\Zed\MerchantProductStorage\Communication\Plugin\Publisher\Merchant |
| MerchantProductWritePublisherPlugin                          | Publishes the product by merchant product abstract IDs to the key-value store (Redis or Valkey). |           | Spryker\Zed\MerchantProductStorage\Communication\Plugin\Publisher\MerchantProduct |
