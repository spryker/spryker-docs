...
To configure export to the key-value store (Redis or Valkey), take the steps in the following section:
...
| PLUGIN | SPECIFICATION | PREREQUISITES | NAMESPACE |
|-|-|-|-|
| SynchronizationStorageQueueMessageProcessorPlugin | Configures all merchant product offers to sync with key-value store (Redis or Valkey), and marks messages as failed in case of error. |   | Spryker\Zed\Synchronization\Communication\Plugin\Queue |
...
Make sure that after setting up the event listeners, `console sync:data product_concrete_product_offer_price` exports data from the `spy_product_concrete_product_offer_price_storage` table to the key-value store (Redis or Valkey).

Make sure that when the following entities get updated through the ORM, the corresponding the key-value store (Redis or Valkey) keys have the correct values.
