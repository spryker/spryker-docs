module Jekyll
  class SidebarTag < Liquid::Tag
    ...
    def render(context)
        ...
        site = @context['site']
        @all_urls = site.pages.map(&:url).to_h { |key| [key, 1] }
    ...
    def build_sidebar_string(sidebar_data)
        ...
        sidebar_data.each do |nested_item|
            ...
            sidebar_item_url = versionize_url(nested_item['url'], @version)
...
EOF
                ...
                if not @all_urls.has_key?(sidebar_item_url) then
                    sidebar_item_url = versionize_url(nested_item['url'], 'latest')
                end
...
EOF
    ...
    def versionize_url(url, version)
