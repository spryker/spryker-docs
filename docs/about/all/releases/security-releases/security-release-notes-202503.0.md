...
3. Add the `SPRYKER_CUSTOMER_REMEMBER_ME_SECRET` environment variable in the cloud. This Spryker-owned variable can only be added or updated with the help of the Spryker Cloud or Support teams. For instructions, see [Add variables in the Parameter Store](/docs/ca/dev/add-variables-in-the-parameter-store.html).
