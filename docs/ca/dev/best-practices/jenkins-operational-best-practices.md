...
Keep in mind that each Jenkins executor can run one PHP job, which may potentially spawn multiple PHP threads or child processes. Each php thread can consume RAM up to `memory_limit` value. 
