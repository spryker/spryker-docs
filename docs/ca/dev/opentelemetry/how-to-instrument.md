...
All hooks that are autogenerated or provided by a library, such as Propel or Redis hooks provided by Spryker, are registered automatically. If you want to register a hook for a class, make sure the registration is executed before the method you want to instrument is called. Hook example:
...
OpenTelemetry provides instrumentation via packages that can be installed and register hooks automatically. If you want to instrument something that is not covered in our code base but is required for you, you can try to install one of the packages listed [here](https://opentelemetry.io/ecosystem/registry/?language=php) or from other sources.
