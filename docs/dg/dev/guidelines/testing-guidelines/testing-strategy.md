...
|---|---|---|---|---|
| [Cypress journey](#cypress-journey) | end-to-end | Available | Can a customer still complete this journey on the assembled system? | tens of seconds to minutes |
...
| [Provider or processor test](#provider-or-processor-test) | unit | Rolling out | Which branch inside this provider or processor runs? | well under a millisecond |
...
A facade test enters at the Business layer and runs the Persistence layer and the real database underneath it. An API contract test enters at the Communication layer of a Glue module and runs the Business and Persistence layers underneath it. This is why an API contract test that asserts a business value is a facade test with an HTTP round-trip in the way: the deeper test type already owns that question and answers it faster.
...
- **Must not.** Assert HTTP status codes, response envelopes, or JSON shape.
...
| Token introspection is stubbed in API contract tests, and so are request header expansion, HTTP client middleware, and transport-level status semantics | The Cypress journeys that log in and browse over real HTTP |
...
| A facade test never goes over HTTP, so status codes, envelope, and JSON shape are not exercised | API contract test for the operation |
...
- **Cypress** proves a journey works on a fully assembled system. When a Cypress test fails and no cheaper test does, apply the push-down heuristic before fixing the Cypress test.
...
- **End-to-end tests doing API work.** A browser or HTTP test that asserts status codes and validation errors for every operation duplicates the API contract suite at many times the cost and none of the precision.
