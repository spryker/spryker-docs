...
redirect_from:
  - /docs/dg/dev/architecture/api-platform/configuration.html
---
...
The simplest starting point is a real, released configuration. The links below point to the B2B Demo Marketplace at release `release-202604.0`; check newer releases for updates.
...
For a full example, see [Enable the documentation UI only in development](#enable-the-documentation-ui-only-in-development).

### Configuration examples

#### Enable the documentation UI only in development

By default, Spryker shows the documentation UI (Swagger UI) only in development environments. You can configure this behavior based on the `$env` parameter:
...
For a complete guide on creating native resources, see [Native API Platform resources](/docs/integrations/spryker-api/api-platform/native-api-platform-resources.html).
...
Security is configured in a separate `security.php` file. For details, see [Integrate API Platform security](/docs/integrations/spryker-api/authenticating-and-authorization/integrate-api-platform-security.html).
