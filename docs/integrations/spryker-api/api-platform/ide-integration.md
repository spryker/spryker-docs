...
`string`, `integer`, `number`, `boolean`, `array`, `object`, `map`, `mixed`. Short aliases `int`, `bool`, `str`, and `arr` are accepted and normalized. Use `map` for structured JSON payloads with a known shape but no dedicated class—it is the recommended type for such payloads, documented via `openapiContext`. For the full type reference, see [Resource schemas — Property types](/docs/integrations/spryker-api/api-platform/resource-schemas.html#property-types).
...
For the full attribute list, see the [resource schemas reference](/docs/integrations/spryker-api/api-platform/resource-schemas.html).
