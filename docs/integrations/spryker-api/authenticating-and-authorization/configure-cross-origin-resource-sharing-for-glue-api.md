...
description: Configure cross-origin resource sharing (CORS) to control which origins can send requests to Glue API endpoints.
...
1. Make an *OPTIONS* pre-flight request to any valid Storefront API resource with the correct `Origin` header, for example, `http://www.example1.com`:
