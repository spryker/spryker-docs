...
- API applications: Out of the box, the Glue infrastructure provides two API applications.
...
- Backend API Applications: The current Glue-based Backend API infrastructure. Use it when creating a new Backend API application. Note that the strategic direction is API Platform—see [Migration status - Glue API to API Platform](/docs/integrations/spryker-api/migrate-from-glue-to-api-platform/migrate-to-api-platform-status.html).
