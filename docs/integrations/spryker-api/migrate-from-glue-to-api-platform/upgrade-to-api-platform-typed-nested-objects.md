...
redirect_from:
  - /docs/dg/dev/upgrade-and-migrate/upgrade-to-api-platform-typed-nested-objects.html
---
...
- API Platform is integrated as described in [Integrate API Platform](/docs/integrations/spryker-api/migrate-from-glue-to-api-platform/integrate-api-platform.html).
...
Note the `allowMissingFields: true` semantics on value objects: an absent field denormalizes to `null`, so lifted `NotBlank` constraints gain `allowNull: true` and `NotNull` constraints are dropped — absent passes, present-but-empty still fails. See [Validation schemas](/docs/integrations/spryker-api/api-platform/validation-schemas.html) for details.
