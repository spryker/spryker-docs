...
description: Learn how to debug and troubleshoot common Spryker API errors.
last_updated: July 30, 2026
template: default
redirect_from:
  - /docs/integrations/spryker-glue-api/spryker-api-errors-and-troubleshooting.html
---
...
The error response format is the same regardless of whether an endpoint is served by the legacy Glue infrastructure or by API Platform: errors are returned as JSON:API error objects with the Glue-compatible numeric `code` field. Everything on this page applies to both infrastructures unless stated otherwise.
...
- For endpoints served by Glue: ensure the `ResourceRoutePlugin` is correctly defined and registered in the appropriate dependency provider (`GlueApplicationDependencyProvider` for legacy, or application-specific for decoupled).
- For endpoints served by API Platform: verify the resource is generated and registered—run `docker/sdk cli glue api:debug --list` and see [API Platform troubleshooting](/docs/integrations/spryker-api/api-platform/troubleshooting.html).
