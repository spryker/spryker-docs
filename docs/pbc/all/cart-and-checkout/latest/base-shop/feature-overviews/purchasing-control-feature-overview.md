...
| Warn | A warning is displayed to the buyer, but they can proceed. The order still requires approval. |
...
3. **Orders are tagged.** At checkout, the buyer selects which cost center the purchase is charged to. The system automatically selects the active budget for that cost center:
   - If exactly one active budget fits—it is selected automatically.
   - If multiple budgets fit—the buyer selects from a dropdown.
