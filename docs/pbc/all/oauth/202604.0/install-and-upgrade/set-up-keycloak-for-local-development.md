...
Add the following to the `image.environment` section of `deploy.dev.yml`. The client names (`SSO_YVES`, `SSO_ZED`, `SSO_MP`) follow the `SPRYKER_OAUTH_{CLIENT_NAME}_{FIELD}` convention described in [Add an OAuth provider](/docs/pbc/all/oauth/{{page.version}}/install-and-upgrade/add-an-oauth-provider.html).
