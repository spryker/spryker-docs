...
- **Can the ERP or pricing engine handle that load** at the target latency and availability, if you intend to call it live? Get real numbers from the ERP team rather than assumptions. This single answer decides whether live pricing is even on the table. See [Performance guidelines: External HTTP requests](/docs/dg/dev/guidelines/performance-guidelines/external-http-requests.html), which explains why Spryker reads from fast storage and what to check before you deviate.
...
|---|---|---|---|
| S1 | Remove prices from Elasticsearch | 🟢 Low | Exclude price data from the product abstract search document by removing the price map from the product page search plugin stack, and resolve prices from the key-value store on the catalog and product details page instead, lazy-loading them over AJAX where needed (see [Asynchronous AJAX calls](/docs/dg/dev/guidelines/performance-guidelines/external-http-requests.html#asynchronous-ajax-calls-to-avoid-blocking-page-rendering)). Price sorting and price-range facets become unavailable. See also [Search performance guidelines](/docs/dg/dev/guidelines/performance-guidelines/search-performance-guidelines.html). |
| S2 | Company-level pricing (not BU-level) | 🟢 Low | Associate merchant relationships with the company instead of each business unit, and use the company identifier as the price dimension key during price resolution. Reduces the record count by the average number of business units per company, typically 3–10×. For how merchant relationship prices work, see [Merchant Custom Prices feature overview](/docs/pbc/all/price-management/latest/base-shop/merchant-custom-prices-feature-overview.html). |
...
| S4 | Optimize price data import | 🟡 Medium | Replace row-by-row writes with batched statements of 500–1,000 rows (see [Batch processing of Propel entities](/docs/dg/dev/guidelines/performance-guidelines/performance-guidelines-batch-processing-propel-entities.html)), write only rows that actually changed by tracking a timestamp or version hash on the source side, pre-load merchant relationship, currency, and store lookups once per batch instead of per row, and stream rows with PHP generators to keep memory flat. See also [Database performance guidelines](/docs/dg/dev/guidelines/performance-guidelines/database-performance-guidelines.html). |
| S5 | Optimize the Publish and Sync pipeline | 🟢 Low | Batch the key-value store writes, accumulate and deduplicate publish events per product so one logical change publishes once, increase the queue consumer chunk size, and pre-load relational lookups once per event batch to remove per-event queries from the publisher. Consider [splitting the publish queues](/docs/dg/dev/guidelines/performance-guidelines/split-queues-performance.html) so price events no longer share a queue with everything else; for the pipeline itself, see [Publish and Synchronization](/docs/dg/dev/backend-development/data-manipulation/data-publishing/publish-and-synchronization.html). |
| S6 | ERP live fetch with a Redis TTL cache | 🟡 Medium | Add a price dimension expander plugin that reads a dedicated key namespace in the key-value store — [Create personalized prices](/docs/pbc/all/price-management/latest/base-shop/tutorials-and-howtos/create-personalized-prices.html) shows the price dimension extension points. On a miss, call the ERP price API through a dedicated client, batching every missing SKU of the current page or cart into one request (see [Replace N+1 with collection endpoints](/docs/dg/dev/guidelines/performance-guidelines/external-http-requests.html#replace-n1-with-collection-endpoints)), and store the result under a single configurable TTL. Add a circuit breaker that serves the last-known price when the ERP is unreachable, and plan cache persistence and warm-up, because the cache is now the only price store. For the cart-side plugin, see [Implementing ERP Integration](/docs/integrations/custom-building-integrations/erp-integration-template.html). |
...
| S8 | Single price for abstract and concrete | 🟢 Low | Link price records to the abstract product only and let concrete products read through to the abstract price — the default [price retrieving logic](/docs/pbc/all/price-management/latest/base-shop/prices-feature-overview/prices-feature-overview.html#price-retrieving-logic) already falls back this way — applying this equally to standard, merchant relationship, and volume prices. Halves the record count in the database, cache, and search index. Keep the current per-variant model as an explicit opt-in for products whose variants genuinely carry different prices. |
| S9 | Price group abstraction (price lists) | 🔴 High | Introduce a custom price dimension for a named price group — for example, "Tier A customer" or "Automotive OEM" — map each company to a group through a dedicated join table, and resolve company to group to price, layering customer-specific overrides on top for key-account exceptions. Requires the ERP to export prices per group rather than per customer, so agree it with the ERP team first. For the price dimension extension points, see [Create personalized prices](/docs/pbc/all/price-management/latest/base-shop/tutorials-and-howtos/create-personalized-prices.html). |
...
| O5 | Pre-resolve the buy-box at publish time | 🟡 Medium | The [Buy Box feature](/docs/pbc/all/offer-management/latest/marketplace/buy-box-feature-overview.html) already ranks offers by price or stock availability, but it does so on every read. Compute the winning offer by your chosen strategy — lowest price, in stock, seller rating, or delivery time — whenever offers change instead, and store only the winner, or the top N, per concrete and dimension in a small dedicated key that the catalog listing and product details page read directly. Recompute only when a relevant offer attribute changes. |
...
| O7 | Live/TTL offer fetch | 🟡 Medium | Apply the **S6** demand-driven pattern to offers: on a cache miss, batch-fetch the buy-box offer with its price and stock from the seller or marketplace-operator system and cache it under a configurable TTL, with a circuit breaker to the last-known offer plus the **S7** staleness indicator when the source is unreachable (see [Performance guidelines: External HTTP requests](/docs/dg/dev/guidelines/performance-guidelines/external-http-requests.html)). Plan cache persistence and warm-up, because the cache becomes the authoritative offer store. |
