...
Usually, employees within a company have different roles (for example, purchasing, administration, supervision). These roles are related to company users and are referred to as **Company Roles**. A role can be default (the **is_default** option), which means that it is applied to all new users automatically.

Upon the initial creation of the first company user, the default role is Admin. After the Admin user has been created, they can create the structure of the company and define the default roles to be further used.
...
Permissions that can be used are not limited in any way. You can create and integrate any permissions. Each permission is represented as a plugin in the code.
...
* Because of this, one of the roles within the company is an administrator who has all the permissions with regards to the company profile's creation and management, user accounts, and user rights.
...
The permissions giving the user more rights have priority. For example, suppose junior sales managers are allowed to place an order for up to 1000 Euros, whereas team leaders can place orders for up to 2000 Euros. If a user has both roles assigned to him, they can place orders for up to 2000 Euros.

As another example, if junior sales managers can't view specific products, but team leaders can, the users which have both a junior sales manager and team leader roles can view those products.
...
## Permission types
Permissions can be both simple and complex.
<table>
	<tr>
	...
	<tr>
        <td><b>Complex</b></td>
		<td>Complex permissions have at least some logic behind them and answer the question "Does the customer have a permission with some parameters and business logic?". Complex permissions implement ExecutablePermissionPluginInterface (Shared).</td>
		<td>
...
* Yves permissions do not need to get any data from the database. They refer to key-value storage or can search to check the right for actions.
...
For example, the permission to add to a cart up to X [order value] is a Zed-side permission. In this case, the process of permissions check is as follows:
...
The termination hooks (plugin stack) prevent permissions from sneaking into the business logic foundation, keeping it clean from permissions, and not overwhelmed with "can" "if not; then…".
...
Every company role includes a set of permissions that can be enabled/disabled according to your needs by switching the toggle in **Enable** column:
 ...
 - **Add company users**—allows adding company users. With this permission enabled, a user has the **Create User** button on the **Company Users** page.
  ...
  - **Place Order**—allows placing the order. Without this permission enabled, a user gets an error when trying to submit the order. If the [Approval Process](/docs/pbc/all/cart-and-checkout/{{page.version}}/approval-process-feature-overview.html) feature is integrated into your project, then **Buy up to grand total** permission is also required to place an order.
  - **Buy up to grand total (Requires "Send cart for approval")**—sets a limit for the grand total of the cart. If the amount in the cart is larger than the limit set in this permission, the user can't proceed to checkout. Works with **Send cart for approval** permission. This permission is available after enabling the [Approval Process](/docs/pbc/all/cart-and-checkout/{{page.version}}/approval-process-feature-overview.html) feature.
  - **Approve up to grand total**—with this permission enabled, a user can approve the cart. For more details, see [Approval Feature Overview](/docs/pbc/all/cart-and-checkout/{{page.version}}/approval-process-feature-overview.html).
  ...
  - **Send cart for approval (Requires "Buy up to grand total")**—lets a user send the cart for approval. Works together with Buy up to grand total permission. See [Approval Feature Overview](/docs/pbc/all/cart-and-checkout/{{page.version}}/approval-process-feature-overview.html) for more details.
