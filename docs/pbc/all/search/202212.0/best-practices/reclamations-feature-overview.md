...
Handling order claims is the reality of doing business for most e-commerce sites. A customer might have bought an item and then discovered that it does not fit their needs, or they might have stumbled upon another item and want to order it instead of the already placed order, or an item is faulty, or additional parts are needed, etc.

The _Reclamations_ feature provides a way for administrators to handle all the customer order claims. The shop administrators can create, view, and edit the reclamations in the dedicated section of the Administration Interface. The editing implies making a refund (depending on your state machine configuration) or closing the reclamation. Thus, the Reclamation feature makes the process of handling claims easier, faster, and more efficient for the store administrators, as well as helping improve customer service.
