...
| priceMode | Useful if your store is configured with multiple price modes. Return the prices defined for the price mode value provided | {% raw %}{{{% endraw %}price_mode_name{% raw %}}}{% endraw %}<br>Spryker provides two price modes by default: `GROSS_MODE` and `NET_MODE`. See this [documentation](https://docs.spryker.com/docs/pbc/all/price-management/{{page.version}}/extend-and-customize/configuration-of-price-modes-and-types.html) to learn more |
...
| `GET https://glue.mysprykershop.com/catalog-search?priceMode=NET_MODE` | Specify that you want returned only prices defined in the `NET_MODE` |
