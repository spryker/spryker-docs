...
This document explains how to configure faceted navigation, filters, pagination, and sorting, which are the most critical features provided by the `Search` module. 

This configuration is only relevant if you enable the three available query expanders and the result formatters. For details, see [Configure a search query](/docs/pbc/all/search/{{page.version}}/tutorials-and-howtos/configure-a-search-query.html)
...
Let's assume that previously in your `PageMapInterface` you mapped an integer facet called "price" with some data already input (note the use of `addIntegerFacet()` in the previous example). You now want to add a price range filter for that data.
...
* The next method you implement is the `buildSortConfig()`, where you can configure your sorting options. Let's assume you want to sort by name and price, and you've already added them when implementing `PageMapInterface` (check the use of `addStringSort()` and `addIntegerSort()` in the example above).
...
After providing the instance of your search configuration builder, the *expander* and *result formatter* plugins start generating data the next time you run a search query. This tutorial doesn't cover how to display the filters, but you can find examples using them in our Demo Shops.
