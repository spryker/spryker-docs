...
- Your project uses `spryker/stock-address` 1.5.0 or later. `StockAddressAclEntityConfigurationExpanderPlugin` was introduced in version 1.5.0.
...
1. Make sure that the warehouse assigned to one of the merchant's product offers has an address. In the database, the `spy_stock_address` table must contain a record whose `fk_stock` points to that warehouse.
...
4. Make sure that the operation completes and that the order moves to the expected state.
5. Make sure that no `NullValueException` for a missing address is logged for the request.
