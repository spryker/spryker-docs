...
To quickly populate an external system like ERP or OMS with data from your Spryker shop, you can export it as files from the Spryker shop and then import them into the external system.
...
2. Specify necessary configurations in the YAML export configuration file residing in `./data/export/config/`. For details about the YAML export configuration file structure and configuration options, see [.yml Export Configuration File](#yml-export-configuration-file).
3. Run `console data:export --config file-name.yml`, where `file-name.yml` is the name of the YAML export configuration file. The command creates export CSV files in the `./data/export/`folder for each *data_entity* of the YAML file. For each store specified in the YAML file, a separate file is created. For an example of how the export works, see [Structure of the .yml Export Configuration File](/docs/scos/dev/data-export/{{page.version}}/data-export.html#structure).
...
## YAML export configuration file

The YAML export configuration file lets you define what orders you want to export. The following content is exported:

* order
* order-item
* order-expense

By default, the YAML export configuration file resides in `./data/export/config/`. You can adjust your YAML export configuration file, but when doing so, stick to its [structure](/docs/scos/dev/data-export/{{page.version}}/data-export.html#structure) and take the possible [data filtering options](/docs/scos/dev/data-export/{{page.version}}/data-export.html#filter) into account.
...
### Structure of the YAML export configuration file

Structure of the YAML export configuration file is as follows:
...
actions:  
   ...
   -  data_entity: order-expense             
      ...
      filter_criteria:
          ...
          store_name: [<store_name_value_1>]
```
Type of content to export is defined in section *actions* by *data_entity* and must be *order*, *order-item* and *order-expense* . You can define what stores you want to run export for, and specify order dates you want to export data for. For the infomration about how to export order data for specific stores and time period, see [Setting the Filter Criteria](/docs/scos/dev/data-export/{{page.version}}/data-export.html#filter) in a YAML file.

For example, check out the default YAML export configuration file [order_export_config.yml](https://github.com/spryker-shop/suite/blob/master/data/export/production/order_export_config.yml). It’s configuration presupposes batch export of the three data entities: *order*, *order-item*, *order-expense.*
...
### Setting the filter criteria in a YAML file

You can set the following filter criteria for the order data export in your YAML export configuration file:
...
For example, if you want to export the *order-expenses* data for the DE store, and *order-items* data for DE and AT stores, your YAML file should look like this:
...
And vice versa: if you want to overwrite the existing files, remove *{timestamp}* from the *destination* parameter of the YAML file for the necessary *data_entity* items, for example:
