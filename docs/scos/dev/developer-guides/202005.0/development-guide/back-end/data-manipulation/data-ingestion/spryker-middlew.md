...
* KeyMapeRule—the simplest rule for the mapper that gets the value via the key from the payload. You can use the `.` symbol as a key separator for getting value from the payload.
