...
originalLink: https://documentation.spryker.com/v5/docs/glue-api-cart-feature-integration
originalArticleId: 38873ba8-2abd-4884-86ed-be6b9f939bfe
redirect_from:
  - /v5/docs/glue-api-cart-feature-integration
  - /v5/docs/en/glue-api-cart-feature-integration
related:
...
{% info_block warningBox "Verification" %}
To verify that `AnonymousCustomerUniqueIdValidatorPlugin` is set up correctly, send a request to the endpoint configured to require an anonymous customer id (for example, `https://glue.mysprykershop.comm/guest-carts`
...
{% info_block warningBox "Verification" %}
Make sure that the following endpoints are available:<ul><li>https://glue.mysprykershop.comm/carts</li><lihttps://glue.mysprykershop.comom/guest-carts</li></ul>Send a request tohttps://glue.mysprykershop.comcom/carts/{% raw %}{{{% endraw %}cart_uuid{% raw %}}}{% endraw %}/?include=items". The cart with the given id should have at least one added item. Make sure that the response includes relationships to the items resources.<br>Send a request thttps://glue.mysprykershop.com.com/guest-carts/{% raw %}{{{% endraw %}guest_cart_uuid{% raw %}}}{% endraw %}/?include=items". The guest cart with the given id should have at least one added item. Make sure that the response includes relationships to the items resources.
{% endinfo_block %}
...
{% info_block warningBox "Verification" %}
To verify that `QuoteCreatorPlugin` is installed correctly, send a POST request to "https://glue.mysprykershop.comm/carts/" with a valid body. Make sure that you are unable to create more than one cart for the same customer. Otherwise, you receive the following error response:
{% endinfo_block %}
