...
The following schema represents the configuration process flow of the configurable product:
...
The following sections describe phases the process flow is consists of.

## Phase 1

  - We are on a product details page (PDP)—the page with the product configuration. <!-- replace we => you -->
...
We will generate URL that points to the gateway page with the following parameters: <!-- we => you -->
...
2. The configurator handles the request by creating a new session and saving the request data there (phase 4.2).
...
2. In the backend, the response is prepared according to the public data API from configurator to Spryker. (phase 6.2)
...
1. The gateway page receives data. <!--the following steps seem incorrect-->
2. The data is checked through the execution of the validator plugins stack for received data.
  - If validation is not successful that the request is redirected to the `backUrl` without saving the <!--the what?-->  of the configuration with a warning message.
3. If the validation part is successful that the configuration is saved to the session.
...
## Product configuration data flow on Yves when configuration starts from the Cart page.

 Product configuration data flow consists of the following phases.

### Phase 1:
- We are on Cart—the page with items that contain the product configuration.
- The product configuration can be already complete or not.
- The item product configuration can be taken from one source only:
  - Quote.
- We generate the URL that points to the gateway page with the following parameters.
...
The client clicks the configuration button, and the form request is redirected to the gateway page with a given product configuration using a GET method.
### Phase 3
...
3. The plugin expands the request with additional necessary data:
  - store, locale, currency, customer, price mode.
  - `CheckSum`—it’s the calculates the crc32 polynomial of a request data as a string.
  - `timestamp`—the timestamp when the request is created.
  - `backUrl`—based on the referer header.
  - The plugin will generate the URL that points to the configurator page together with all the necessary data.
### Phase 4
...
2. The data is being checked through the execution of the validator plugins stack for received data.
  - If validation is not successful that the request is redirected to the `backUrl` without saving the of the configuration with a warning message.
3. If validation is successful then updates the cart item configuration in the cart.
4. All applicable plugins that can handle the configurator response are executed.
