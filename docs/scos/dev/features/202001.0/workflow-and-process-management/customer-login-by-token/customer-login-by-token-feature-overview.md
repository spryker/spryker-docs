...
In Spryker Commerce OS, token generation is performed using a facade method, that is why no GUI is present. To generate a token, see [HowTo - Generate a Token for Login](https://documentation.spryker.com/v4/docs/ht-generating-token-for-login-201907).

Token-based authentication works closely with the [Punch Out](https://documentation.spryker.com/v4/docs/punchout-201907) feature. It allows B2B buyers to log in from their ERP system to a Spryker company user account using a token without entering the username and password and buy the products from Spryker e-commerce shop.
