...
Request sample: `POST https://glue.mysprykershop.com/agent-customer-impersonation-access-tokens`
...
```json
{
    "data": {
        ...
        "links": {
            "self": "https://glue.mysprykershop.com/agent-customer-impersonation-access-tokens"
        }
    }
}
...
| refreshToken | String | Token used to [refresh](/docs/scos/dev/glue-api-guides/{{page.version}}/managing-agent-assists/managing-agent-assist-authentication-tokens.html#refresh-an-agent-assist-authentication-token) the `accessToken`. |
