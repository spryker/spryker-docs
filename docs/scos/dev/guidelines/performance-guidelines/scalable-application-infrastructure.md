...
At the beginning of the processing, queues contain a different number of messages. To process all messages efficiently, processes need to be distributed among queues according to the number of messages per queue. === So how do I make sure they are distributed?

> :info: `max_children` value must be reviewed in **php-fpm** server configuration according to configured `THREAD_POOL_SIZE`. === What for?
