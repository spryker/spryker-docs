...
If there are no MIME types defined in the **MIME Type Settings** section, you can download any type of file. If you have at least one MIME type defined as Is Allowed, you can download only the files of that type unless you add more allowed types. For more details, see [Managing MIME Type Settings](/docs/scos/user/back-office-user-guides/{{page.version}}/administration/mime-type-settings/managing-mime-type-settings.html).
...
To create a file directory, do the following:
...
The created folder is displayed on the left of the **Overview of File Tree** page.
...
To manage a file, do the following:
...
2. In the **Files List** section, select one of the following in the **Actions** column for a file:
    * To view the file, select **View**. The **View file** page opens where you can download it by clicking **Download** in the **Actions** column.
    * To edit the file **Edit**. The **Edit file** page opens where you can update the file information. You can even download another file instead of the one that is already in use.
    * To delete a file, select **Delete**.
