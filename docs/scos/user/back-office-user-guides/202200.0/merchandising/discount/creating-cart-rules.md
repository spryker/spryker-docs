...
Name and describe discounts in a way that is meaningful for other Back Office users. Besides, the given name is displayed in the customer's cart. Therefore, it must be unique.
...
Conditions are also called *decision rules*. A cart rule can have one or more conditions linked to it. The cart rule is redeemed only if every condition linked to it is satisfied.
...
<br>To create a discount that will have an extensive number of conditions, you use the condition **groups**. Meaning you collect different rules under different groups and split them into separate chunks.
