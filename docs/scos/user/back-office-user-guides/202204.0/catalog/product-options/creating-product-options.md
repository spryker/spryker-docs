...
. Select a **TAX SET**.
. Enter a **1. OPTION NAME TRANSLATION KEY**.
. Enter a **SKU**.
. For **PRICES**, enter the needed prices for the option.
. Optional: To add one more option, click **Add option** and repeat steps - .
. Repeat steps - until you add the needed options.


. In the **Translation** section, enter a **GROUP NAME** per locale.
. Enter an **OPTION NAME** for each option per locale.
