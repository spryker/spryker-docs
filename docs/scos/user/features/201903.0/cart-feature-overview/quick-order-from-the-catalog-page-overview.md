...
Quick Order quantity rules fall under limitations set by [quantity restrictions](/docs/scos/user/features/{{page.version}}/product-management/product-quantity-restrictions/product-quantity-restrictions.html). Quantity restriction values define the number of items that customer can put into the cart.
...
The price is displayed dynamically taking into account the quantity, currency, store and [merchant relation](/docs/scos/user/features/{{page.version}}/merchant-b2b-contracts-feature-overview.html) dimensions respectively. If the item has any [volume prices](/docs/scos/user/features/{{page.version}}/prices-feature-overview/volume-prices-overview.md), the price is adjusted to the correct price set for the quantity the user has selected.
...
In case the quantity is not valid due to [quantity restrictions](/docs/scos/user/features/{{page.version}}/product-management/product-quantity-restrictions/product-quantity-restrictions.html) setting, the quantity is changed to a valid number.
