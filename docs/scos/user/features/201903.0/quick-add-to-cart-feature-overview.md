...
The price is displayed dynamically, taking into account the quantity, currency, store, and [merchant relation](docs\scos\user\features\202009.0\merchant-b2b-contracts\merchant-b2b-contracts.md) dimensions, respectively. If the item has any [volume prices](//docs/scos/user/features/{{page.version}}/prices-feature-overview/volume-prices-overview.html), the price is adjusted to the correct price set for the quantity the user has selected.
