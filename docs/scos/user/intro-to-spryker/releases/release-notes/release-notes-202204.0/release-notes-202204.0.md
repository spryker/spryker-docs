...
We added several new features and improvements to discounts to allow for more control and transparency around your promotions.
...
When the **Cart** page shows eligible promotional products, these products now show the discounted price with a strikethrough default price. This improves the customer experience by showing the discount a customer can receive if they add the product to the cart.
...
Wherever a number is displayed, it is according to the current locale of the session of the guest or customer.

### Back-Office

Wherever a number is displayed, it is according to the current locale of the user’s session.
...
Users can now export the data from the tables of the Back Office (or the merchant portal). I can export the fields that I see or (that are available on a Back Office table) in a CSV file using the **Export** button.
As a developer, I can enable a business user to export a table view within a few hours per a single table view by assigning an exporting functionality (for example, plugin or trait) to a Table Query container or Repository.

## PHP compatibility window

As we have previously communicated, we are deprecating PHP 7.3 compatibility to maintain a “healthy” support window on the allowed PHP versions (Supported versions of PHP | Spryker Documentation). We continue to work on the upcoming versions, and we have solved some bugs that were blocking full PHP 8.0 compatibility. Our main focus is on 8.1 preparation, especially on Propel library support.

## Symfony compatibility window

We have also made compatible Symfony 5.4, while forward-looking Symfony 6 for compatibility purposes, our compatibility window deprecates Symfony 3 as 3 versions are maintained.
...
We have enhanced several points where performance has been improved to allow for a faster page load, and better throughput amongst the main revisited items are as follows:

* Upselling products, previously loaded synchronously, are now be loaded via AJAX.
* Cart items are also loaded into the cart using AJAX calls.
* Checking availability for all current SKUs at once instead of one by one.
* On the **Cart** page, we do a Zed call to validate the Quote. Inside of that, some queries were executed more than once.
* Gateway and ZedRestApi are not using the same resources as a "normal" Zed request; separating those reduces the required ApplicationPlugins and improves performance.
