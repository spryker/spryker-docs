...
3. Authentication is handled with the help of the [Customer login by token](/docs/scos/user/features/company-account-feature-overview/customer-login-by-token-overview.html) feature. So, the buyer doesn't need to enter any login details to get to the customer account. The [Vault for Tokens](/docs/scos/user/features/spryker-core-feature-overview/vault-for-tokens-overview.html) feature allows securely keeping the sensitive data (username and password) and retrieving it on request.
