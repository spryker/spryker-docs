...
2. Populate the necessary fields in the **General, Price & Tax, Variants, SEO, Image** tabs. See [Creating an Abstract Product](/docs/scos/dev/user-guides/202001.0/back-office-user-guide/products/products/abstract-products/creating-an-abs) for details on the tabs and their values.
...
5. Make sure your product is visible searchable in the Storefront by going through the checklist in the HowTo - [Make a Product Searchable and Shown on the Storefront](https://documentation.spryker.com/v4/docs/ht-make-product-shown-on-frontend-by-url#howto---make-a-product-searchable-and-shown-on-the-storefront) article.
...
{% endinfo_block %}
You can make the service, in our example, the installation service, a [product option](/docs/scos/dev/user-guides/202001.0/back-office-user-guide/products/product-options/product-options) of the product you sell. In this case, the installation service will not be a separate product, but an optional part of the product. This means that buyers can decide on their own if they need the service. 
...
2. Populate all necessary fields in **General** tab. See [Creating a Product Option](https://documentation.spryker.com/v4/docs/creating-a-product-option#creating-a-product-option) for details on the fields and their values.
...
{% endinfo_block %}
One more way to have a product with an optional service offering would be to make the product a [configurable bundle](/docs/scos/dev/features/202001.0/product-information-management/configurable-bundle/configurable-bu). It means that you create a product (for example, equipment) and suggest another product (Installation service) as a bundle. To create such a configurable bundle product, follow the steps below.

1. Create the *Installation Service* product as a usual abstract product, like you did in the [Prerequisites](https://documentation.spryker.com/v4/docs/creating-service-offerings#prerequisites) section. Make sure you set the **Availability** as *Never out of stock*. See [Managing Products Availability](https://documentation.spryker.com/v4/docs/managing-products-availability#managing-products-availability) for details on how to do that.
{% info_block infoBox "Note" %}
...
{% endinfo_block %}
2. Create a [Configurable Bundle Template](https://documentation.spryker.com/v4/docs/configurable-bundle-feature-overview#configurable-bundle-template) with two [slots](https://documentation.spryker.com/v4/docs/configurable-bundle-feature-overview#configurable-bundle-slots) - one for the physical products, and one for the service:
    ...
    7. Assign the entire categories or individual physical products to the slot. Make sure to assign all products for which you want to offer the specific service. See [Adding Products to a Slot](https://documentation.spryker.com/v4/docs/managing-configurable-bundle-templates#editing-the-slot-for-a-configurable-bundle-template) for details on how to assign categories and products.
